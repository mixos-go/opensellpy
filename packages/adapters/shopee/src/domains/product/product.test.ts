...
test('toShopeeListProductsParams menghitung offset dan default item_status', () => {
  ...
  // tanpa updatedFrom/updatedTo → tidak ada window waktu (item lama ikut terlihat)
  assert.equal('update_time_from' in params, false)
  assert.equal('update_time_to' in params, false)
  const withStatus = toShopeeListProductsParams({ page: 1, limit: 10, status: 'draft' })
  assert.equal(withStatus['item_status'], 'REVIEWING')
  const withWindow = toShopeeListProductsParams({ page: 1, limit: 10, updatedFrom: 1700000000, updatedTo: 1700000100 })
  assert.equal(withWindow['update_time_from'], 1700000000)
  assert.equal(withWindow['update_time_to'], 1700000100)
})